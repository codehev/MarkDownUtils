...
def setup_logger(log_file: str):
    ...
    # 控制台日志处理器（只显示进度条，不显示日志）
    console_handler = logging.StreamHandler()
    console_handler.setLevel(logging.CRITICAL)  # 设置为 CRITICAL 级别，避免输出日志
...
主要优化点：
日志写入文件：
使用 logging.FileHandler 将日志写入文件。
日志文件使用 UTF-8 编码，避免中文乱码。

控制台只显示进度条：
控制台日志处理器设置为 logging.CRITICAL 级别，避免输出日志。
使用 tqdm 显示进度条。
...
日志文件路径：
日志文件默认保存在当前工作目录下的 markdown_image_downloader.log。
"""
